...
void ClusterizeGridDebug(Bool_t isLocal, TString geomFile = "geometry.txt", TString paraFile = "parameters.txt", TString inDir = "", TString outDir = "") 
{
    TFile* f = new TFile(Form("%sclusterizerHistograms.root",outDir.Data()),"RECREATE");
    TH2F* hCalibratedCls = new TH2F("hCalibratedCls","",100,-50.,+50.,100,-50.,+50.);
    TH2F* hXCl_EnCl_CoarseBefore = new TH2F("hXCl_EnCl_CoarseBefore","",100,-50.,+50.,100,0.,+2e6);
    TH2F* hYCl_EnCl_CoarseBefore = new TH2F("hYCl_EnCl_CoarseBefore","",100,-50.,+50.,100,0.,+2e6);
    TH2F* hXCl_EnCl_FineBefore = new TH2F("hXCl_EnCl_FineBefore","",100,-50.,+50.,100,0.,+2e3);
    TH2F* hYCl_EnCl_FineBefore = new TH2F("hYCl_EnCl_FineBefore","",100,-50.,+50.,100,0.,+2e3);
    TH2F* hXCl_EnCl_CoarseAfter = new TH2F("hXCl_EnCl_CoarseAfter","",100,-50.,+50.,100,0.,+200.);
    TH2F* hYCl_EnCl_CoarseAfter = new TH2F("hYCl_EnCl_CoarseAfter","",100,-50.,+50.,100,0.,+200.);
    TH2F* hXCl_EnCl_FineAfter = new TH2F("hXCl_EnCl_FineAfter","",100,-50.,+50.,100,0.,+200.);
    TH2F* hYCl_EnCl_FineAfter = new TH2F("hYCl_EnCl_FineAfter","",100,-50.,+50.,100,0.,+200.);
    gROOT->cd();
    ...
    Float_t* paramsCoarse = clusterizer->GetCalibrationPars(COARSE_CALIB);
    cout << "\n\n\nCOARSE_CALIB:  " << paramsCoarse[0] << ",  " << paramsCoarse[1] << ",  " << paramsCoarse[2] << "\n";
    Float_t* paramsFine = clusterizer->GetCalibrationPars(FINE_CALIB);
    cout << "FINE_CALIB:  " << paramsFine[0] << ",  " << paramsFine[1] << ",  " << paramsFine[2] << "\n\n\n";
    ...
    // Create clusterMaps
    if (clusterizer->GetDebugMode()) cout << "Creating cluster maps..." << endl; 
    ...
    for (Int_t ievt = 0; ievt < runLoader->GetNumberOfEvents(); ievt++) 
    {
        ...
        // clusterization for second pixel layer
        ...
        for(Int_t segment = 0; segment < nSegments; segment++) clusterMap[segment]->ResetMap();
            
        Int_t nSubClusters = subClusters->GetEntries();
            
        if(clusterizer->GetDebugMode()) cout << "Registering clusters into maps..." << endl;
        ...
        for (Int_t c = 0; c < nSubClusters; c++) 
        {
            ...
            if ((segment < 0) || (segment >=nSegments)) // weird segment
                continue;
            ...
            if (!clusterMap[segment]->Get(col,row)) clusterMap[segment]->InsertAt(col,row,subCluster);
            else {
                Bool_t found = false;
                ringer.SetRing(1);
                Int_t xCol,yCol;
        ...
        if (clusterizer->GetDebugMode()) cout << "Combining clusters..." << endl;
        ...
        {  
            ...
            if (!isPixelSegment) {
                finalSegment = -2;
                if (clusterizer->GetDebugMode()) cout << "COARSE CLUSTERS:" << endl;
                map = coarseClusterMap;
                calibSegment = COARSE_CALIB;
                if (firstCoarseSeed == -1) firstCoarseSeed = ALL_SEED;
            } else {
                finalSegment = -3;
                if (clusterizer->GetDebugMode()) cout << "FINE CLUSTERS:" << endl;
                map = fineClusterMap;
            ...
            for (Int_t c = 0; c < nSubClusters; c++) 
            {
                ...
                for (Int_t segment2 = 0; segment2 < nSegments; segment2++) 
                {
                    ...
                    if (geometry->GetVirtualIsPixel(segment2) != isPixelSegment) continue;
                    ...
                    for (Int_t ring = 0; ring <= 3; ring++) 
                    {
                        ...
                        while(ringer.GetNext(xCol,yCol)) 
                        {
                            ...
                            {
                                if (merged && !multipleMerges) multipleMerges = true;
                ...
                if(calibSegment == COARSE_CALIB) {
                    hXCl_EnCl_CoarseBefore->Fill(x/energy,energy);
                    hYCl_EnCl_CoarseBefore->Fill(y/energy,energy);
                } else {
                    hXCl_EnCl_FineBefore->Fill(x/energy,energy);
                    hYCl_EnCl_FineBefore->Fill(y/energy,energy);
                }
                Float_t* pars = clusterizer->GetCalibrationPars(calibSegment);
                Float_t calibratedEnergy = Calibration(energy,calibSegment,pars,clusteringCalibrationRun,clusterizer->GetDebugMode());
                hCalibratedCls->Fill(x/energy,y/energy);
                if(calibSegment == COARSE_CALIB) {
                    hXCl_EnCl_CoarseAfter->Fill(x/energy,calibratedEnergy);
                    hYCl_EnCl_CoarseAfter->Fill(y/energy,calibratedEnergy);
                } else {
                    hXCl_EnCl_FineAfter->Fill(x/energy,calibratedEnergy);
                    hYCl_EnCl_FineAfter->Fill(y/energy,calibratedEnergy);
                }
                ...
                while(ringer.GetNext(xCol,yCol)) 
                {
                    ...
                    if (!closest) {
                        ...
                        if (actualDistance < closestDistance) {
                            ...
                            if (wasFoundPreviously) wasFoundPreviously = false;
                        }
                    }
                } // end ringer while
        ...
        if (clusterizer->GetDebugMode()) cout << "  CREATING NEW CLUSTERS FROM FINE ONES:" << endl;
        ...
        for (Int_t c = nSubClusters; c < nSubClusters2; c++) 
        {
            ...
            for (Int_t i = 0; i < AliFOCALCluster::N_DEPTH_FIELDS; i++) 
            {
                if (subCluster->GetSegIndex(i) >= 0) {
	                ...
	                if (clusterizer->GetDebugMode()) cout << Form("\t\t\tAdding info: Segment=%i\tWidth1=%.2f\tWidth2=%.2f\tLongE=%.2f",
	                ...
	                if (clusterizer->GetDebugMode()) cout << Form("\t\t\tAdding info (cl2): Segment=%i\tWidth1=%.2f\tWidth2=%.2f\tLongE=%.2f/%.2f",
                        newCluster->GetSegIndex(i),newCluster->GetWidth1(i),
	    ...
	    if (clusterizer->GetDebugMode()) cout << "  CREATING NEW CLUSTERS FROM LONELY COARSE ONES:" << endl;
        ...
        if (clusterizer->GetDebugMode()) cout << "  REMOVE CLUSTERS WITH 0 ENERGY AND CREATE FINAL LIST" << endl;
        ...
        if (clusteringSaveImages) clusterizer->SaveImage(Form("event_%02i",ievt));
    } // end events
...
Float_t Calibration(Float_t signal, Int_t segment, Float_t* pars, bool calibrationRun, bool debug) 
{
    ...
    if(powerBase < 0) 
    {
        if (debug) cout << Form("\t\tCalibration in segment %i: Negative power base (%.2e), returning 0",segment,powerBase) << endl;
        result = 0;
    }
    else result = pars[0]*TMath::Power(powerBase,pars[2]) + pars[3];
    */
    Float_t result = (signal - pars[1]*(signal/pars[0]*signal/pars[0]))/pars[0];
  
    if(result < 0) {
        if (debug) cout << Form("\t\tCalibration in segment %i: Negative value (%.2e), returning 0",segment,result) << endl;
        result = 0;
    }
    if(debug) cout << Form("\t\tCalibration in segment %i from %.2e to %.2e GeV",segment,signal,result) << endl;
    return result;
}
